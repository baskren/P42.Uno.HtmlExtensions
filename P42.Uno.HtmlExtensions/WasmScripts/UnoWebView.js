...
function UnoWebView_PostMessage(id, message) {
    console.log('UnoWebView.js[' + UnoWebView_InstanceGuid +']_PostMessage ENTER [' + id + ']: ' + message.substring(0,100));
    let m = JSON.parse(message);
    let target = document.getElementById(id);
    target.contentWindow.postMessage(m, "*");
    console.log('UnoWebView.js[' + UnoWebView_InstanceGuid +']_PostMessage EXIT [' + id + ']: ' + message.substring(0,100));
}

function UnoWebView_SetMessageListener() {
    if (UnoWebView_MessageListenerSet) {
        console.log('UnoWebView.js[' + UnoWebView_InstanceGuid + ']_SetMessageListener: ALREADY SET');
        return;
    }
    window.addEventListener("message", (event) => {
        ...
        if (!ignore) {
            console.log('UnoWebView.js[' + UnoWebView_InstanceGuid +']: messageListener ENTER : ' + JSON.stringify(event.data));
            console.log('UnoWebView.js[' + UnoWebView_InstanceGuid +']: messageListener href: ' + window.location.href);
            ...
            console.log('UnoWebView.js[' + UnoWebView_InstanceGuid +']: messageListener EXIT : ' + JSON.stringify(event.data));
        }
    }, false);
}

function UnoWebView_OnLoad(index) {
    console.log('UnoWebView.js[' + UnoWebView_InstanceGuid +']_OnLoad: ENTER ' + index);
    ...
    OnFrameLoaded(index);
    console.log('UnoWebView.js[' + UnoWebView_InstanceGuid +']_OnLoad: EXIT ' + index);
}
